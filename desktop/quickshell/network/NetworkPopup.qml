...
Item {
    ...
    Shortcut {
        sequence: "Tab"
        onActivated: {
            if (window.activeMode === "wifi") window.activeMode = "bt";
            else if (window.activeMode === "bt") window.activeMode = "wired";
            else window.activeMode = "wifi";
        }
    }
    ...
    readonly property color wiredAccent: Qt.lighter(window.sapphire, 1.15)

    property string activeMode: "bt"
    readonly property color activeColor: activeMode === "wifi" ? window.wifiAccent : (activeMode === "bt" ? window.btAccent : window.wiredAccent)
    ...
    function syncCores() {
        let list = activeMode === "wired" ? [] : (activeMode === "wifi" ? (isWifiConn && wifiConnected ? [window.wifiConnected] : []) : window.btConnected);
    ...
    property var wiredList: []
    property bool networkingEnabled: true
    property bool wiredPowerPending: false
    readonly property bool isWiredConn: window.wiredList.some(function(i) { return i.isConnected; })

    readonly property bool currentPower: activeMode === "wired" ? window.networkingEnabled : (activeMode === "wifi" ? window.wifiPower === "on" : window.btPower === "on")
    onCurrentPowerChanged: { syncCores(); }

    readonly property bool currentPowerPending: activeMode === "wired" ? window.wiredPowerPending : (activeMode === "wifi" ? window.wifiPowerPending : window.btPowerPending)
    readonly property bool currentConn: activeMode === "wired" ? window.isWiredConn : (activeMode === "wifi" ? window.isWifiConn : window.isBtConn)
    ...
    function processWiredJson(textData) {
        if (!textData) return;
        try {
            let data = JSON.parse(textData);
            window.networkingEnabled = data.networking === "enabled";
            window.wiredPowerPending = false;
            let ifaces = data.interfaces || [];
            window.wiredList = ifaces;
            window.syncModel(wiredListModel, ifaces);
        } catch(e) {}
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                processWifiJson(this.text.trim());
            }
        }
    }
    ...
    Process {
        id: wiredPoller
        command: ["bash", window.scriptsDir + "/wired_panel_logic.sh"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                processWiredJson(this.text.trim());
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (!wiredPoller.running) wiredPoller.running = true;
        }
    }
    ...
    Item {
        ...
        Rectangle {
            ...
            Item {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        property real multiShift: (window.activeMode === "wifi" || window.activeMode === "wired") ? 0.0 : window.multiTransitionState
                        ...
                        Rectangle {
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    ...
                                    text: window.activeMode === "wifi" ? "󰤮" : (window.activeMode === "wired" ? "󰈂" : "󰂲")
                                }
                                Text {
                                    Layout.alignment: Qt.AlignHCenter
                                    ...
                                    text: window.currentPowerPending
                                        ? (window.activeMode === "wired" ? "Updating..." : ((window.activeMode === "wifi" ? window.expectedWifiPower : window.expectedBtPower) === "on" ? "Powering On..." : "Powering Off..."))
                                        : (!window.currentPower ? (window.activeMode === "wired" ? "Networking Off" : "Radio Offline") : (window.activeMode === "wired" ? "Offline" : "Scanning..."))
                                }
                            }
                            ...
                            Item {
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        ...
                                        text: window.activeMode === "wired" ? "󰈀" : (isMyDisconnecting ? "" : (coreMa.containsMouse ? (window.activeMode === "wifi" ? "󰖪" : "󰂲") : (coreContainer.myDevice ? coreContainer.myDevice.icon : "")))
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                    LoadingDots { Layout.alignment: Qt.AlignHCenter; visible: isMyDisconnecting && window.activeMode !== "wired"; dotCol: window.overlay1 }
                                    Text {
                                        Layout.alignment: Qt.AlignHCenter
                                        ...
                                        color: isMyDisconnecting ? window.overlay1 : window.crust
                                        text: window.activeMode === "wired" ? "Ethernet" : (coreContainer.myDevice ? (window.activeMode === "wifi" ? coreContainer.myDevice.ssid : coreContainer.myDevice.name) : "")
                                    ...
                                    Text {
                                        ...
                                        text: window.activeMode === "wired" ? "Online" : (isMyDisconnecting ? "Disconnecting..." : (centralCore.disconnectFill > 0.01 ? "Hold..." : "Connected"))
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                }
                                ...
                                Item {
                                    ...
                                    ColumnLayout {
                                        ...
                                        LoadingDots { Layout.alignment: Qt.AlignHCenter; visible: isMyDisconnecting && window.activeMode !== "wired"; dotCol: window.text }
                                        Text {
                                            Layout.alignment: Qt.AlignHCenter
                                            Layout.maximumWidth: 150 - (50 * coreContainer.multiShift)
                                            horizontalAlignment: Text.AlignHCenter
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                cursorShape: window.currentConn && !isMyDisconnecting && window.activeMode !== "wired" ? Qt.PointingHandCursor : Qt.ArrowCursor

                                onPressed: {
                                    if (window.activeMode !== "wired" && window.currentConn && !isMyDisconnecting && !centralCore.disconnectTriggered) {
                                        coreDrainAnim.stop();
                                        coreFillAnim.start();
                                    }
                                }
                                onReleased: {
                                    if (window.activeMode !== "wired" && !centralCore.disconnectTriggered && !isMyDisconnecting) {
                                        coreFillAnim.stop();
                                        coreDrainAnim.start();
                                    }
                                }
                ...
                Item {
                    ...
                    Repeater {
                        id: orbitRepeater
                        model: window.activeMode === "wired" ? wiredListModel : ((window.currentConn && window.showInfoView) ? infoListModel : (window.activeMode === "wifi" ? wifiListModel : btListModel))
                        ...
                        delegate: Item {
                            ...
                            property real unifiedRatio: (window.activeMode === "wifi" || window.activeMode === "wired") ? 0.0 : window.multiTransitionState
                            ...
                            property real safeMultiShift: (window.activeMode === "wifi" || window.activeMode === "wired") ? 0.0 : window.multiTransitionState
                                ...
                                RowLayout {
                                    ...
                                    ColumnLayout {
                                        ...
                                        Text {
                                            ...
                                            text: floatCard.isMyBusy ? (model.isConnected ? "Disconnecting..." : "Connecting...") : (floatCard.renderFill > 0.1 && floatCard.renderFill < 1.0 ? "Hold..." : action)
                                            Behavior on color { ColorAnimation { duration: 200 } }
                                        }
                                    }
                                }
                                ...
                                NumberAnimation {
                                    ...
                                    onFinished: {
                                        ...
                                        if (cmdStr === "TOGGLE_VIEW") {
                                            ...
                                            drainAnim.start();
                                        } else if (window.activeMode === "wired" && cmdStr) {
                                            let wdd = window.disconnectingDevices;
                                            wdd[floatCard.itemId] = true;
                                            window.disconnectingDevices = Object.assign({}, wdd);
                                            Quickshell.execDetached(["sh", "-c", cmdStr]);
                                            wiredPoller.running = true;
                                            floatCard.triggered = false;
                                            drainAnim.start();
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        Layout.fillHeight: true
                        radius: 10
                        color: window.activeMode === "wired" ? "transparent" : (wiredTabMa.containsMouse ? window.surface1 : "transparent")
                        Behavior on color { ColorAnimation { duration: 200 } }

                        Rectangle {
                            anchors.fill: parent
                            radius: 10
                            opacity: window.activeMode === "wired" ? 1.0 : 0.0
                            Behavior on opacity { NumberAnimation { duration: 300 } }
                            gradient: Gradient {
                                orientation: Gradient.Horizontal
                                GradientStop { position: 0.0; color: Qt.lighter(window.wiredAccent, 1.15) }
                                GradientStop { position: 1.0; color: window.wiredAccent }
                            }
                        }
                        ...
                        RowLayout {
                            anchors.centerIn: parent
                            spacing: 8
                            Text { font.family: "Iosevka Nerd Font"; font.pixelSize: 18; color: window.activeMode === "wired" ? window.crust : window.text; text: "󰈀"; Behavior on color { ColorAnimation{duration:200} } }
                            Text { font.family: "JetBrains Mono"; font.weight: Font.Black; font.pixelSize: 13; color: window.activeMode === "wired" ? window.crust : window.text; text: "Wired"; Behavior on color { ColorAnimation{duration:200} } }
                        }
                        MouseArea {
                            id: wiredTabMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                window.activeMode = "wired";
                            }
                        }
                    }
                }
            ...
            // ─── Connection Info Button (top-left) ───────────────────
            Rectangle {
                id: connInfoBtn
                anchors.top: parent.top
                anchors.left: parent.left
                ...
                width: 300
                height: expanded ? (36 + infoDetails.implicitHeight + 8) : 36
                ...
                Item {
                    ...
                    Row {
                        ...
                        anchors.right: infoChevron.left
                        anchors.rightMargin: 6
                        spacing: 8
                        Text {
                            font.family: "Iosevka Nerd Font"; font.pixelSize: 16
                            color: window.currentConn ? window.activeColor : window.overlay0
                            anchors.verticalCenter: parent.verticalCenter
                            Behavior on color { ColorAnimation { duration: 200 } }
                            text: {
                                if (window.activeMode === "wifi") return window.isWifiConn && window.wifiConnected ? window.wifiConnected.icon : "󰤮"
                                if (window.activeMode === "wired") return window.isWiredConn ? "󰈀" : "󰈂"
                                return window.isBtConn ? "󰂯" : "󰂲"
                            }
                        }
                        Text {
                            font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: 12
                            color: window.currentConn ? window.text : window.overlay0
                            anchors.verticalCenter: parent.verticalCenter
                            Behavior on color { ColorAnimation { duration: 200 } }
                            elide: Text.ElideRight; width: 200
                            text: {
                                if (window.activeMode === "wifi")
                                    return window.wifiConnected ? window.wifiConnected.ssid : "Not connected"
                                if (window.activeMode === "wired") {
                                    let c = window.wiredList.find(function(i) { return i.isConnected; })
                                    return c ? c.name : "Disconnected"
                                }
                                if (window.btConnected.length === 1) return window.btConnected[0].name
                                if (window.btConnected.length > 1) return window.btConnected.length + " devices"
                    ...
                    Text {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        font.family: "Iosevka Nerd Font"; font.pixelSize: 13; color: window.overlay0
                        text: connInfoBtn.expanded ? "󰅃" : "󰅀"
                    }
                    MouseArea {
                        anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                        onClicked: connInfoBtn.expanded = !connInfoBtn.expanded
                    }
                }

                // helper component: one label+value row
                component InfoRow : Row {
                    property string label: ""
                    property string value: ""
                    property bool show: value !== "" && value !== "--"
                    visible: show
                    width: parent ? parent.width : 0
                    spacing: 0
                    Text {
                        width: 110; font.family: "JetBrains Mono"; font.pixelSize: 11
                        color: window.overlay1; text: label
                    }
                    Text {
                        width: parent.width - 110; font.family: "JetBrains Mono"; font.pixelSize: 11
                        color: window.text; text: value; elide: Text.ElideRight
                ...
                component SectionHeader : Item {
                    ...
                    Rectangle { anchors.bottom: parent.bottom; width: parent.width; height: 1; color: window.surface0 }
                    Text {
                        anchors.bottom: parent.bottom; anchors.bottomMargin: 5
                        font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: 10
                        color: window.overlay0; text: parent.title.toUpperCase(); font.letterSpacing: 1
                    }
                }
                ...
                Column {
                    ...
                    topPadding: 6; bottomPadding: 10; spacing: 3

                    // ── WIFI ───────────────────────────────────────
                    Loader {
                        active: window.activeMode === "wifi"
                        width: parent.width
                        sourceComponent: Column {
                            spacing: 3; width: parent ? parent.width : 0
                            SectionHeader { title: "General" }
                            InfoRow { label: "SSID";     value: window.wifiConnected ? window.wifiConnected.ssid : "" }
                            InfoRow { label: "Signal";   value: window.wifiConnected ? window.wifiConnected.signal + "%" : "" }
                            InfoRow { label: "Security"; value: window.wifiConnected ? (window.wifiConnected.security || "Open") : "" }
                            InfoRow { label: "MAC";      value: window.wifiConnected ? (window.wifiConnected.mac || "") : "" }
                        }
                    }

                    // ── BT ─────────────────────────────────────────
                    Loader {
                        active: window.activeMode === "bt"
                        width: parent.width
                        sourceComponent: Column {
                            spacing: 3; width: parent ? parent.width : 0
                            SectionHeader { title: "Connected devices" }
                            Repeater {
                                model: window.btConnected
                                Column {
                                    spacing: 3; width: parent ? parent.width : 0
                                    InfoRow { label: "Name";    value: modelData.name || "" }
                                    InfoRow { label: "MAC";     value: modelData.mac  || "" }
                                    InfoRow { label: "Battery"; value: modelData.battery ? modelData.battery + "%" : "" }
                                    InfoRow { label: "Profile"; value: modelData.profile || "" }
                                    Rectangle { width: parent.width; height: 1; color: window.surface0; visible: index < window.btConnected.length - 1 }
                                }
                            }
                            Text {
                                visible: window.btConnected.length === 0
                                font.family: "JetBrains Mono"; font.pixelSize: 11; color: window.overlay0
                    ...
                    // ── WIRED ──────────────────────────────────────
                    Loader {
                        active: window.activeMode === "wired"
                        width: parent.width
                        sourceComponent: Column {
                            spacing: 3; width: parent ? parent.width : 0
                            Repeater {
                                model: window.wiredList
                                Column {
                                    spacing: 3; width: parent ? parent.width : 0

                                    SectionHeader { title: "General" }
                                    InfoRow { label: "Interface";  value: "Ethernet [" + modelData.name + "]" }
                                    InfoRow { label: "MAC";        value: modelData.mac    || "" }
                                    InfoRow { label: "Driver";     value: modelData.driver || "" }
                                    InfoRow { label: "Speed";      value: modelData.speed  || "" }
                                    InfoRow { label: "Security";   value: "None" }

                                    SectionHeader { title: "IPv4"; visible: modelData.isConnected }
                                    InfoRow { label: "Address";    value: modelData.ip        || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "Broadcast";  value: modelData.broadcast || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "Subnet";     value: modelData.subnet    || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "Gateway";    value: modelData.gateway   || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "DNS";        value: modelData.dns       || ""; show: modelData.isConnected && value !== "" }

                                    SectionHeader { title: "IPv6"; visible: modelData.isConnected && modelData.ipv6 !== "" }
                                    InfoRow { label: "Address";  value: modelData.ipv6    || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "Gateway";  value: modelData.ipv6gw  || ""; show: modelData.isConnected && value !== "" }
                                    InfoRow { label: "DNS";      value: modelData.ipv6dns || ""; show: modelData.isConnected && value !== "" }
                                }
                            }
                        }
                    }
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        if (window.activeMode === "wired") {
                            if (window.wiredPowerPending) return;
                            window.wiredPowerPending = true;
                            Quickshell.execDetached(["nmcli", "networking", window.networkingEnabled ? "off" : "on"]);
                            wiredPoller.running = true;
